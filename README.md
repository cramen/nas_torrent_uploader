...
services:
  torrent-uploader:
    ...
    ports:
      - "3300:3000"
    volumes:
      - /путь/к/вашим/торрент/папкам:/targets
    environment:
      - PORT=3000
```

3. Замените `/путь/к/вашим/торрент/папкам` на реальный путь к директориям на вашем NAS, где хранятся папки для торрентов.
...
5. В открывшемся окне выберите директорию назначения на NAS
6. Нажмите "Загрузить"
...
- Для корректной работы на NAS должны быть созданы директории, в которые вы собираетесь загружать торренты
- Для настройки автоматического добавления загруженных торрентов в ваш торрент-клиент на NAS, настройте наблюдение за директориями назначения в вашем торрент-клиенте